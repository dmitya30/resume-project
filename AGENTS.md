...
- The modern-project inventory is stored in `docs/PROJECT_INVENTORY.md`; a limited public evidence review was completed on 02.09.2026. Initial job-market research was completed on 03.09.2026 and recorded in `docs/MARKET_RESEARCH.md` and `docs/REQUIREMENTS_MATRIX.md`. The selected lines are «Инженер по автоматизации и AI-интеграциям» and «IT-инженер / системный администратор». The next phase is the HH profile audit followed by two new resumes; only selected AICS evidence, external-payment confirmation and minor project contradictions remain open.
