...
## Структура данных генерации письма

Результат workflow 04 сохраняется внутри `analysis_json.cover_letter_generation`.

Основные поля:

- `model` — использованная модель;
- `generated_at` — время генерации;
- `finish_reason` — причина завершения ответа;
- `facts_used` — нормализованный список использованных фактов;
- `model_warnings` — диагностические замечания генеративной модели о соответствии вакансии;
- `warnings` — детерминированные предупреждения по публичному тексту письма;
- `requires_review` — признак необходимости ручной проверки;
